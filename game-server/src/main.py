...
def download_addons(COLLECTION_ID, APP_ID, STEAMCMD_PATH, REPO_PATH):
    ...
    if response.status_code != 200:
        print(f"Failed to get collection details from Steam API. Status code: {response.status_code}")
    ...
    file_ids = [file['publishedfileid'] for file in data['response']['collectiondetails'][0]['children']]

    # Run SteamCMD to download the addons
    for file_id in file_ids:
        subprocess.call([STEAMCMD_PATH, '+force_install_dir', ARMA3_PATH, '+login', 'innovationstudios', 'S_vkhb5hj2', '+workshop_download_item', APP_ID, file_id, '+quit'])

    # Move downloaded mod folders to the repo folder
    for file_id in file_ids:
        mod_folder = os.path.join(ARMA3_PATH, 'steamapps', 'workshop', 'content', APP_ID, file_id)
